    ...
    <ContentTemplate>
        <div class="subheader">
            <h1 class="subheader-title">
                <%= Resources.labels.ratecodemangement %>
            </h1>
        </div>
        <div id="divError">
                                    ...
                                    <div class="col-sm-10 col-xs-12">
                                        <div class="row">
                                            <div class="form-group col-sm-6 col-xs-12">
                                                <label class="col-sm-4 control-label col-xs-12">RATE CODE </label>
                                                <div class="col-sm-8 col-xs-12">
                                                    <asp:TextBox ID="txtRateCode" CssClass="form-control" runat="server" MaxLength="10"/>
                                                </div>
                                            </div>
                                            <div class="form-group col-sm-6 col-xs-12">
        ...
        <div id="divResult" runat="server">
            ...
            <asp:GridView ID="gvRateCodeList" CssClass="table table-hover" runat="server" BackColor="White"
                          BorderColor="#CCCCCC" BorderStyle="Solid" BorderWidth="1px" CellPadding="3"
                          Width="100%"
                          AutoGenerateColumns="False"
                          OnRowDataBound="gvRateCodeList_OnRowDataBound"
                          PageSize="15"
                          OnRowCommand="gvRateCodeList_OnRowCommand"
                          OnRowDeleting="gvRateCodeList_OnRowDeleting">
                <Columns>
                    <asp:TemplateField>
                        <ItemTemplate>
...
<script>
    function SelectCbx(obj) {
        ...
        var count = document.getElementById('<%= gvRateCodeList.ClientID %>').rows.length;
        var elements = document.getElementById('<%= gvRateCodeList.ClientID %>').rows;
        if (obj.checked) {
            for (i = 0; i < count; i++) {
                if (elements[i].cells[0].children[0].type == 'checkbox' && elements[i].cells[0].children[0].id != 'ctl00_ctl20_gvRateCodeList_ctl01_cbxSelectAll') {
    ...
    function ChildClick(CheckBox) {
        ...
        var grid = document.getElementById('<%= gvRateCodeList.ClientID %>');
