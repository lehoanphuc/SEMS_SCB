...
public partial class Widgets_SEMSCurrency_Widget : WidgetBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            txtCurrencyName.Visible = false;
            txtCurrencyNumber.Visible = false;
            txtCurrencyMasterName.Visible = false;
    ...
    private void BindData()
    {
        try
        {
            ...
            int pageSize = gvRateCodeList.PageSize;
            int pageIndex = gvRateCodeList.PageIndex;
            int recordIndex = pageSize * pageIndex;
            litError.Text = string.Empty;
            DataTable dtCurrency = new Currency().SearchRateCode(Utility.KillSqlInjection(txtRateCode.Text.Trim()), pageSize, recordIndex, ref IPCERRORCODE, ref IPCERRORDESC).Tables[0];
            if (IPCERRORCODE == "0")
            {
                gvRateCodeList.DataSource = dtCurrency;
                gvRateCodeList.DataBind();
            }
            else
            {
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (GridViewRow gvr in gvRateCodeList.Rows)
            {
    ...
    protected void gvRateCodeList_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            CheckBox cbxSelect;
    ...
    protected void gvRateCodeList_OnRowCommand(object sender, GridViewCommandEventArgs e)
    {
    ...
    protected void gvRateCodeList_OnRowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            string commandArg = string.Empty;
            commandArg = ((LinkButton) gvRateCodeList.Rows[e.RowIndex].Cells[1].FindControl("lbCCYID")).CommandArgument;
    ...
    protected void btnSearch_OnClick(object sender, EventArgs e)
    {
        try
        {
            ...
            gvRateCodeList.PageIndex = !string.IsNullOrEmpty(SelectedPageNo) ? Convert.ToInt32(SelectedPageNo) - 1 : 0;
            BindData();
        }
        catch (Exception ex)
        {
    ...
    private void GridViewPagingClick(object sender, EventArgs e)
    {
        gvRateCodeList.PageSize =
            Convert.ToInt32(((DropDownList) GridViewPaging.FindControl("PageRowSize")).SelectedValue);
        gvRateCodeList.PageIndex = Convert.ToInt32(((TextBox) GridViewPaging.FindControl("SelectedPageNo")).Text) - 1;
        BindData();
    }
}