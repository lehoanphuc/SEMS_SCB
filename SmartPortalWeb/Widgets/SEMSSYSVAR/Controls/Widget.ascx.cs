...
public partial class Widgets_EBASYSVAR_Control_Widget : WidgetBase
{
            ...
            case IPC.ACTIONPAGE.EDIT:
                try
                {
                    if (CheckPermitPageAction(IPC.ACTIONPAGE.EDIT))
                    {
                        ...
                        if (txtValueName.Text.Equals("COT")|| txtValueName.Text.Equals("BOT"))
                        {
                            if (!IsTimeInDay(txtValueCode.Text))
                            {
                                lblError.Text = Resources.labels.requiretimeofday;
                                return;
                            }
                        }
                        valname = SmartPortal.Common.Utilities.Utility.KillSqlInjection(txtValueName.Text.Trim());
    ...
    public static bool IsTimeInDay(string time)
    {
        return Regex.Match(time, "^(((([0-1][0-9])|(2[0-3])):?[0-5][0-9]:?[0-5][0-9]+$))", RegexOptions.IgnoreCase).Success;
    }
}