...
namespace SmartPortal.SEMS
{
    public class Branch
    {
        ...
        //nhanpt
        public DataSet GetWorkflowBankStaffByTrans(string transactionId, ref string errorCode, ref string errorDesc)
        {
            try
            {
                DataSet ds = new DataSet();
                Hashtable hasInput = new Hashtable
                {
                    {IPC.SOURCEID, IPC.SOURCEIDVALUE},
                    {IPC.SOURCETRANREF, IPC.SOURCETRANREFVALUE},
                    {IPC.IPCTRANCODE, "SEMSLOADWORKFLOW"},
                    {IPC.TRANSID, transactionId},
                };
                Hashtable hasOutput = AT.DBTRAN().ProcessTransHAS(hasInput);
